...
- **Web UI: Training Progress is now actually wired up.** `connectTrainingSSE()` and `updateProgressBar()` existed in the code but were never called from anywhere — `startTraining()` only ever showed a static "Running, PID: X". Starting a run now connects live logs and a progress-polling loop immediately; the progress bar shows a real percentage/ETA when computable (local `.jsonl` dataset + a concrete `batch_size`), otherwise step count and speed without a false percentage.
- **Web UI: Pause/Resume for a running training process**, via `SIGSTOP`/`SIGCONT` on the subprocess. This suspends compute, not VRAM (model/optimizer state stay allocated) — documented as such in the UI and in `/api/train/pause`'s docstring. `soup train`'s normal `Ctrl+C` behavior is untouched.
- **`ExperimentTracker.find_run_by_pid()`** + a `tracker.mark_running(run_id, pid=os.getpid())` call in `soup train` itself, so `/api/train/progress` can find a run's live step/loss/speed from just the subprocess PID the Web UI has — nothing in the frontend previously tracked a `run_id` at all.
- **Web UI: phase detection** (`/api/train/progress` now returns a plain-language phase — "Loading config", "Verifying datasets", "Loading dataset", "Training", "Paused" — parsed from real log markers already printed by `commands/train.py`, plus the tracker's own "has this run logged a metrics row yet" signal for the reliable "Training" phase).
- **Web UI: Compress section merged into New Training** (was a separate top-level page) — collapsed by default so it doesn't crowd the page for people who just want to train.
- **Web UI: Calculator section** — estimates checkpoint size after quantization and/or the Compress section's merge/SVD reduction, using the same `model_size_from_name()` helper the CLI's GPU-sizing logic already relies on. New `/api/calculator/model-size` endpoint.
- **Web UI: Help & Tutorial page** — a step-by-step, beginner-oriented walkthrough (what Soup is, first-training-run steps, what each New Training section does, phases/pause explained, a quantization-format decision table, when Compress is/isn't needed, HF Hub filter semantics, common problems). Separate from Quick Reference, which stays a fast lookup table for people who already know what they're doing.

### Changed

- **Web UI: "Model Hub" renamed to "HF Hub"** throughout (nav item, page title, code comments) — clearer since it covers datasets and benchmarks too, not just models.
- **Web UI: HF Hub Language filter now actually works for model search.** It was shown on the Models tab but silently dropped server-side (`list_models` has no native `language=` kwarg, unlike `list_datasets`) — now composed as a `language:<code>` tag filter, the same mechanism already used for library/license.
- **Web UI: HF Hub Task filter now shows tab-appropriate suggestions** (models use `pipeline_tag` vocabulary, datasets/benchmarks use `task_categories` — different taxonomies that silently returned zero results when crossed). Free-text entry is unrestricted; the `<datalist>` suggestions just point at the right vocabulary per tab.
- **Web UI: Quick Reference moved to the bottom of the New Training page** (was squeezed next to Training Status) and expanded to match the current schema — the old list was missing most task/format/quantization options and had gone stale.
- **Web UI: `/api/train/status` and `/api/train/progress` now include `paused`, `phase`, and `total_steps`/`eta_seconds`** (best-effort, from a new `_estimate_total_steps` — computed from `epochs × dataset rows ÷ effective batch size` when the dataset is a local `.jsonl` file and `batch_size` isn't `"auto"`; returns `None`/"unknown" otherwise rather than guessing).

### Fixed

- **`/api/train/start` used to pipe the training subprocess's stdout and never read it.** Once the OS pipe buffer filled (~64KB on Linux) with nobody watching the (previously unwired) logs page, the process would block on its next `print()` and hang silently forever — indistinguishable from "still training" in `/api/train/status`. An always-on background thread now drains `stdout` into a shared buffer for the lifetime of every training run, whether or not anyone ever opens the logs view; `/api/train/logs` (SSE) now reads from that buffer instead of racing the drain thread for the same pipe.
- A SIGSTOP'd (paused) process can't act on `SIGTERM` until resumed — `/api/train/stop` now sends `SIGCONT` first if the run was paused, so Stop actually takes effect instead of leaving a stopped process behind.
